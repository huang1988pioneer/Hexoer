...
public partial class DeployViewModel : PageViewModelBase
{
    ...
    {
        _services = services;
        GitHubToken = services.Project.Settings.GitHubToken ?? string.Empty;
    ...
    private async Task CheckPagesStatusAsync()
    {
        try
        {
            ...
            {
                _services.Project.Settings.GitHubToken = GitHubToken.Trim();
    ...
    {
        _services.Project.Settings.GitHubToken = string.IsNullOrWhiteSpace(GitHubToken) ? null : GitHubToken.Trim();
        _services.Project.Settings.Save();
        StatusMessage = "已儲存 GitHub Token（本機）";
    }
