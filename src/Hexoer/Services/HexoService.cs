...
public sealed class HexoService
{
    ...
    public async Task<CommandResult> InstallHexoCliGlobalAsync(CancellationToken ct = default) =>
        await _runner.RunShellAsync("npm install -g hexo-cli", cancellationToken: ct).ConfigureAwait(false);
    ...
    {
        // Prefer local hexo via npx
        var result = await _runner.RunShellAsync($"npx --yes hexo {args}", _context.ProjectPath, ct).ConfigureAwait(false);
        if (result.ExitCode == 0 || result.ExitCode == -1)
            return result;

        // Fallback to global hexo
        return await _runner.RunShellAsync($"hexo {args}", _context.ProjectPath, ct).ConfigureAwait(false);
    }
